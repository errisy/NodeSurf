...
var Atom = (function () {
    ...
    Object.defineProperty(Atom.prototype, "isHydrophilic", {
        get: function () {
            return this.name == 'O' || this.name == 'N';
        },
        enumerable: true,
        configurable: true
    });
...
var SurfaceSearchOptions = (function () {
    function SurfaceSearchOptions(hydrophilicFactor, hydrophobicFactor, testRadius, defaultRadius) {
        this.hydrophilicFactor = 0.75;
        this.hydrophobicFactor = 1.333333;
        /**
        * The radius used for water;
        */
        this.testRadius = 0.168;
        /**
        * Radius for default atom depth;
        */
        this.defaultRadius = 0.13;
        if (typeof hydrophilicFactor == 'number' && !isNaN(hydrophilicFactor))
            this.hydrophilicFactor = hydrophilicFactor;
        if (typeof hydrophobicFactor == 'number' && !isNaN(hydrophobicFactor))
            this.hydrophobicFactor = hydrophobicFactor;
        ...
        if (rs.name == 'G') {
            ...
            if (atp) {
                for (var WaterRadius = 0.01; WaterRadius <= 0.332; WaterRadius += 0.005) {
                    ...
                    var ph = new PolyhedronBuilder2(atp, RadiusPseudoG, WaterRadius, options, false); // the builder will create a box at the unit of RadiusAtom * Factor + WaterRadius
                    // the atom can approach AtomRadius * Factor + Diameter of Water
                    var AtomReach = (RadiusPseudoG + WaterRadius) * options.hydrophobicFactor + WaterRadius * options.hydrophobicFactor;
                    var NoRoom = false;
                ...
                switch (at.type) {
                    ...
                    default:
                        ...
                        var _loop_1 = function() {
                            SpecificList = [];
                            atp = at.position;
                            CenterAtomRadius = at.GetRadiusLiMinimumSet; // this is the radius of the atom
                            ph = new PolyhedronBuilder2(atp, CenterAtomRadius, WaterRadius, options, at.isHydrophilic); // the builder will create a box at the unit of RadiusAtom * Factor + WaterRadius
                            //here we need to know whether the atom is hydrophilic or hydrophobic:
                            var AtomReach_1;
                            if (at.isHydrophilic) {
                                AtomReach_1 = (CenterAtomRadius + WaterRadius) * options.hydrophilicFactor + WaterRadius * options.hydrophobicFactor;
                            }
                            else {
                                AtomReach_1 = (CenterAtomRadius + WaterRadius) * options.hydrophobicFactor + WaterRadius * options.hydrophobicFactor;
                            }
                            // the atom can approach AtomRadius * Factor + Diameter of Water
                            includeList.forEach(function (pat) {
                                var TotalReach = pat.GetRadiusLiMinimumSet * options.hydrophobicFactor + AtomReach_1;
                        ...
                        var SpecificList, atp, CenterAtomRadius, ph;
    ...
    SurfaceSearch.Test = function (entry, options) {
        ...
        if (rs.name == 'G') {
            ...
            if (atp) {
                ...
                var RadiusPseudoG = SurfaceSearch.RadiusLiMinimumSet('C', 'CG') + WaterRadius;
                var ph = new PolyhedronBuilder2(atp, RadiusPseudoG, WaterRadius, options, false); // the builder will create a box at the unit of RadiusAtom * Factor + WaterRadius
                var AtomReach = (RadiusPseudoG + WaterRadius) * options.hydrophobicFactor + WaterRadius * options.hydrophobicFactor; // the atom can approach AtomRadius * Factor + Diameter of Water
                var NoRoom = false;
                includeList.some(function (pat) {
                    var TotalReach = pat.GetRadiusLiMinimumSet * options.hydrophobicFactor + AtomReach;
                    if (pat.position.subtract(atp).lengthSquared <= TotalReach * TotalReach)
            ...
            rs.atoms.forEach(function (at) {
                switch (at.type) {
                    ...
                    default:
                        ...
                        var ph = new PolyhedronBuilder2(atp, CenterAtomRadius, WaterRadius, options, at.isHydrophilic); // the builder will create a box at the unit of RadiusAtom * Factor + WaterRadius
                        var AtomReach_2;
                        if (at.isHydrophilic) {
                            AtomReach_2 = (CenterAtomRadius + WaterRadius) * options.hydrophilicFactor + WaterRadius * options.hydrophobicFactor;
                        }
                        else {
                            AtomReach_2 = (CenterAtomRadius + WaterRadius) * options.hydrophobicFactor + WaterRadius * options.hydrophobicFactor;
                        } // the atom can approach AtomRadius * Factor + Diameter of Water
                        includeList.forEach(function (pat) {
                            var TotalReach = pat.GetRadiusLiMinimumSet * options.hydrophobicFactor + AtomReach_2;
                            if (pat !== at && pat.position.subtract(atp).lengthSquared < TotalReach * TotalReach)
                                SpecificList.push(pat);
                        });
                        SpecificList.forEach(function (pat) {
